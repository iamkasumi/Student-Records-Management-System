<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $select = "SELECT * FROM `score-g7` WHERE StudentID = '$StudID' ";
        $result = mysqli_query($conn, $select);
        $rows_count = mysqli_num_rows($result);
        
        if ($rows_count > 0) {
            echo "<script> alert('Grades were already uploaded! Want to edit instead?')</script>";
            echo "<script> window.open('../html/fac-manage-grades-7.php', '_self')</script>";
        }
        
        else {

            $insert = "INSERT INTO `score-g7` (StudentID, Filipino, English, Mathematics, Science, MAPEH, AP, ESP, TLE)
