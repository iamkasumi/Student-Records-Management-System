<?php
    ...
    try {
        ...
        $select_query = "SELECT * FROM `grade-8` WHERE ID = '$id'";
        ...
        $delete_query_grade = "DELETE FROM `grade-8` WHERE ID = '$id'";
        $delete_result_grade = mysqli_query($conn, $delete_query_grade);

        if (!$delete_result_grade) {
            throw new Exception("Error deleting record from grade-8: " . mysqli_error($conn));
        }

        // Delete record from score-g8
        $delete_query_score = "DELETE FROM `score-g8` WHERE StudentID = '$id'";
        $delete_result_score = mysqli_query($conn, $delete_query_score);

        if (!$delete_result_score) {
            throw new Exception("Error deleting record from score-g8: " . mysqli_error($conn));
        }
