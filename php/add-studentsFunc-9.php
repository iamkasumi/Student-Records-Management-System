<?php
    ...
    if (isset($_POST['add-submit'])) {
        ...
        $select = "SELECT * FROM `grade-9` WHERE LRN = '$LRN' OR Email = '$Email'";
        $result = mysqli_query($conn, $select);
        $rows_count = mysqli_num_rows($result);
        
        if ($rows_count > 0) {
            echo "<script> alert('Student already exist!')</script>";
            echo "<script> window.open('../html/add-students-9.php', '_self')</script>";
        }
        
        else {

            $insert = "INSERT INTO `grade-9` (LRN, Name, Email, Mobile, Sex, Birthday, Address, Year)
