<?php
    ...
    if (isset($_POST['LRN_restore'])) {
        ...
        $query = mysqli_query($conn, "SELECT * FROM `archive-g8` WHERE `LRN` = '$LRN'");
        $fetch = mysqli_fetch_assoc($query);

        if ($fetch) {
            // Restore record
            $restore_query = "INSERT INTO `grade-8` (Name, LRN, Email, Mobile, Sex, Birthday, Address, Year) 
            ...
            if ($restore_result) {
                ...
                $delete_query = "DELETE FROM `archive-g8` WHERE `LRN`='$LRN'";
                ...
                if ($delete_result) {
                    ...
                    echo "<script>
                        ...
                        window.location.href='../html/archive-8.php';
                        </script>";
                    exit();
                } else {
    ...
    echo "<script>
        alert('$errorMessage');
        window.location.href='../html/manage-stud-8.php';
        </script>";
?>
