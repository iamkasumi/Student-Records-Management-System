<?php
    ...
    try {
        ...
        $select_query = "SELECT * FROM `grade-9` WHERE ID = '$id'";
        ...
        // Archive record
        $archive_query = "INSERT INTO `archive-g9` (Name, LRN, Email, Mobile, Sex, Birthday, Address, Year)
        ...
        $delete_query_grade = "DELETE FROM `grade-9` WHERE ID = '$id'";
        $delete_result_grade = mysqli_query($conn, $delete_query_grade);

        if (!$delete_result_grade) {
            throw new Exception("Error deleting record from grade-9: " . mysqli_error($conn));
        }

        // Delete record from score-g9
        $delete_query_score = "DELETE FROM `score-g9` WHERE StudentID = '$id'";
        $delete_result_score = mysqli_query($conn, $delete_query_score);

        if (!$delete_result_score) {
            throw new Exception("Error deleting record from score-g9: " . mysqli_error($conn));
        }
